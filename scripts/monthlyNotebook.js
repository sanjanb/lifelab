...
function validateMonthlyNotebook(notebook) {
  ...
  if (!Number.isInteger(notebook.year) || notebook.year < 1900 || notebook.year > 2100) {
    throw new Error("Invalid year in notebook");
  }

  if (!Number.isInteger(notebook.month) || notebook.month < 1 || notebook.month > 12) {
  ...
  if (notebook.days.length !== expectedDays) {
    throw new Error(`Expected ${expectedDays} days for ${notebook.year}-${notebook.month}`);
  }
...
function validateDayEntry(day, dayNum) {
  ...
  if (typeof day.domainSignals !== "object" || Array.isArray(day.domainSignals)) {
  ...
  for (const [domainId, value] of Object.entries(day.domainSignals)) {
    if (typeof value !== "boolean") {
      throw new Error(`Day ${dayNum} domainSignals['${domainId}'] must be boolean`);
    }
  }

  // Validate manualOutcome if present
  if (day.manualOutcome !== null && 
      !["win", "neutral", "loss"].includes(day.manualOutcome)) {
    throw new Error(`Day ${dayNum} manualOutcome must be "win", "neutral", "loss", or null`);
  }

  if (day.reflectionNote !== undefined && typeof day.reflectionNote !== "string") {
