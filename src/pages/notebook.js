...
import { renderImportExportUI, exportToCSV, exportToJSON } from '../data/export.js';
...
function renderTableRow(day, domainList) {
  ...
  return `
    <tr>
      ...
      ${scores.map(s => `<td class="score-cell">${typeof s === 'number' ? s.toFixed(2) : s}</td>`).join('')}
      <td class="avg-cell"><strong>${avg !== null ? avg.toFixed(2) : '—'}</strong></td>
      <td class="notes-cell">${truncateNotes(day.notes || '')}</td>
      <td class="actions-cell">
        <button class="btn-icon btn-edit" data-date="${day.date}" title="Edit">✏️</button>
        <button class="btn-icon btn-delete" data-date="${day.date}" title="Delete">🗑️</button>
      </td>
    </tr>
  `;
}
...
function calculateAverage(day) {
  ...
  const scores = Object.values(day.domains).filter(s => typeof s === 'number');
...
function renderExportOptions() {
  ...
  container.querySelector('#export-month-json').addEventListener('click', () => {
    exportToJSON(currentData, currentYear, currentMonth);
  });

  container.querySelector('#export-month-csv').addEventListener('click', () => {
    const filename = `lifelab-${currentYear}-${String(currentMonth).padStart(2, '0')}.csv`;
