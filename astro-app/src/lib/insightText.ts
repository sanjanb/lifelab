...
export function generateInsightObservations(
  ...
  if (gaps.length > 0) {
    const longestGap = gaps.reduce((max, g) =>
...
export function getMonthSummary(insight: MonthlyInsight): string {
  ...
  return `${activeDays} of ${totalDays} days had activity across ${domainCount} domain${
    domainCount !== 1 ? "s" : ""
  }.`;
}

/**
 * PHASE 7: OBSERVATION LANGUAGE CONSTRAINTS
 *
 * All text generated by this module MUST follow these rules:
 *
 * DESCRIPTIVE LANGUAGE (ALLOWED):
 * ✓ "Activity clustered in..."
 * ✓ "Quiet period from..."
 * ✓ "Focus was concentrated in..."
 * ✓ "X domains were active"
 * ✓ "Pattern shifted between..."
 *
 * EVALUATIVE LANGUAGE (FORBIDDEN):
 * ❌ "Good job!"
 * ❌ "You improved..."
 * ❌ "Try to increase..."
 * ❌ "You should..."
 ...
 *
 * ADVICE/PRESCRIPTIVE LANGUAGE (FORBIDDEN):
 * ❌ "Consider adding more..."
 * ❌ "Focus on..."
 * ❌ "Your goal should be..."
 * ❌ "To improve, try..."
 *
 * GAMIFICATION LANGUAGE (FORBIDDEN):
 * ❌ "Streak of X days"
 ...
 * This tool is designed for long-term psychological safety.
 * Over months and years, evaluative language creates:
 * - Guilt during low-activity periods
 * - Pressure to "perform" rather than reflect
 * - Anxiety about maintaining streaks
 * - Gamification that corrupts intrinsic motivation
 *
 * Descriptive language preserves:
 * - Curiosity and self-awareness
 * - Non-judgmental pattern recognition
 * - Sustainable engagement over years
 * - Authentic reflection without shame
 *
 ...
 * - Streak celebrations
 *
 * Ask yourself: "Will this create guilt or judgment after 3 years of use?"
 * If yes, don't add it.
 */
